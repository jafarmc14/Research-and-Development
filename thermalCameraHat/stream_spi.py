...
class ThermalCamera:
    def __init__(self):
        self.emissivity = 0.98  # Emissivity value for human skin
        self.distance = 1.0  # Distance from the camera to the subject's body in meters
        self.ambient_temperature = 21.0  # Ambient temperature in Celsius
        self.calibration_offset = 0.0 

    def calibrate_camera(self, reference_temperature, pixel_temperatures):
        # # Adjust for emissivity, distance, and ambient temperature
        # adjusted_reference_temperature = reference_temperature / self.emissivity
        # adjusted_reference_temperature += (self.distance * 0.1)  # Example correction for distance
        # adjusted_reference_temperature += self.ambient_temperature  # Add ambient temperature
        
        # # Calculate average measured temperature
        # average_measured_temperature = sum(pixel_temperatures) / len(pixel_temperatures)

        # # Calculate calibration offset
        # self.calibration_offset = adjusted_reference_temperature - average_measured_temperature

        # Calculate average measured temperature
        average_measured_temperature = sum(pixel_temperatures) / len(pixel_temperatures)

        # Calculate calibration offset
        self.calibration_offset = reference_temperature - average_measured_temperature

        # Apply calibration offset to ambient temperature
        self.ambient_temperature += self.calibration_offset
        
        return round(self.calibration_offset, 2)

# Simulated reference temperature from a calibrated thermometer
reference_temperature = 35.0  # Celsius
max_temp = 37.2
...
def cv_display(img, title='display', resize=(320, 248),
        ...
        for x in range(img.shape[1]):
            ...
            if pixel_value > 220:
                ...
                if circle_count > max_circles:
                    center = (x * resize[0] // img.shape[1], y * resize[1] // img.shape[0])
                    circle_color = (0, 0, 255)  # Red color
                    circle_radius = 5
                    cv.circle(cvresize, center, circle_radius, circle_color, -1)
                    patientExist = 1

        if circle_count >= max_circles:
            break
    
    cv.imshow("sksksk", cvresize)
    return patientExist
...
while True:
    ...
    if modeCalib :
        calibration_offset = camera.calibrate_camera(reference_temperature, data)

        # Access the calibration offset value
        print("Calibration Offset:", calibration_offset)
        modeCalib = 0
        calibrated_data = data + calibration_offset

        # Apply constraints
        # calibrated_data = np.where(calibrated_data >= max_temp, max_temp, calibrated_data)
        calibrated_data = np.where(data <= calibration_offset, calibration_offset, calibrated_data)

        # print("Original Data:", data[:5])
        # print("Calibrated Data:", calibrated_data[:5])

        print("max data calib", calibrated_data.max())
        print("max data ", data.max())
        print("min data calib", calibrated_data.min())
    ...
    # # Define reference temperatures and corresponding pixel values
    # reference_temperatures = np.array([36.5, 37.0, 36.8, 36.9])  # Example reference temperatures in Celsius
    # reference_pixel_values = np.array([30, 31, 32, 33])  # Example corresponding pixel values from thermal image

    # # Perform linear regression to calibrate the thermal image data
    # m, c = np.polyfit(reference_pixel_values, reference_temperatures, 1)

    # # Apply calibration to the entire thermal image (img)
    # calibrated_temperatures = m * img + c

    # # Print the calibrated temperatures for each pixel
    # for y in range(calibrated_temperatures.shape[0]):
    #     for x in range(calibrated_temperatures.shape[1]):
    #         temperature = calibrated_temperatures[y, x]
    #         print(f"Calibrated temperature at pixel ({x}, {y}): {temperature}")

    # print(len(mi48.fpa_shape))
    # print(type(mi48.fpa_shape))
    # print(mi48.fpa_shape)
    # Print the temperature value of each pixel
    # for y in range(img.shape[0]):
    #     for x in range(img.shape[1]):
    #         temperature = img[y, x]
    #         print(f"Temperature at pixel ({x}, {y}): {temperature + 6}")
    #         if x == 80 and y == 62:
    #             break
    #     if x == 80 and y == 62:
    #         break
    ...
    img8u =  cv.resize(img8u, (320, 248))
    # print(img8u.max())

    # for y in range(img8u.shape[0]):
    #     for x in range(img8u.shape[1]):
    #         temperature = img8u[y, x]
    #         # after nolmalization
    #         print(f"value at pixel ({x}, {y}): {temperature}")

    # corrected_image = cv.cvtColor(img8u, cv.COLOR_BGR2RGB)
    if GUI:
        # cv_display(img8u)
        # cv.imshow("title", img8u)

        patientState = cv_display(img8u)
        ...
        data = {
            "device_name": "PMM_001",
            "token": "2024",
            "message_type": "sensors_data",
            "temperature": round(data.max() + 6, 2),
            "patientState": patientState
        }
        client.publish("testing/data", json.dumps(data))

        # cv.imshow("Image", img8u)
